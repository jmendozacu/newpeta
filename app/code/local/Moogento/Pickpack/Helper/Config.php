...
class Moogento_Pickpack_Helper_Config extends Mage_Core_Helper_Abstract
{
	...
	const FONT_SIZE_BARCODE_ORDER = 14;
	const FONT_SIZE_BARCODE_PRODUCT = 12;
	const NUDGE_BOTTOM_ORDER_ID = 7;
    ...
    public function getGeneralConfigArray($storeId = null) {
        ...
        if(!isset($this->_general[$storeId])) {
            ...
            $this->_general[$storeId]['override_address_format_yn'] = $this->getConfig('override_address_format_yn', 0, false, 'general', $storeId);
            $this->_general[$storeId]['address_format'] = $this->getConfig('address_format', '', false, 'general', $storeId);
            $this->_general[$storeId]['address_countryskip'] = trim(strtolower($this->getConfig('address_countryskip', 0, false, 'general', $storeId)));
			...
			if($this->_general[$storeId]['custom_fonts_yn'] == 1) {
                ...
                $this->_general[$storeId]['fill_bkg_color_comments'] = trim($this->getConfig('fill_bkg_color_comments', self::FILL_BKG_MESSAGE_YN, false, 'general', $storeId));
...
//			}
    ...
    public function getPackingsheetConfigArray($wonder = 'wonder', $storeId = null) {
        ...
        if(!isset($this->_packingsheet[$storeId][$wonder])) {
            ...
            if ($this->_packingsheet[$storeId][$wonder]['letterhead']){
                ...
                if ($this->_packingsheet[$storeId][$wonder]['pickpack_logo_position'] != 'fullwidth')
                ...
                else $this->_packingsheet[$storeId][$wonder]['pickpack_company_address_yn'] = 0;
            }else{
                $this->_packingsheet[$storeId][$wonder]['pickpack_packlogo'] = 0;
                $this->_packingsheet[$storeId][$wonder]['pickpack_company_address_yn'] = 0;
            }
            ...
            $this->_packingsheet[$storeId][$wonder]['pickpack_title_pattern_nudge'] = explode(',', $this->getConfig('title_pattern_nudge', '0,0', false, $wonder, $storeId));
            $this->_packingsheet[$storeId][$wonder]['billing_details_yn'] = $this->getConfig('billing_details_yn', 0, false, $wonder, $storeId);
            $this->_packingsheet[$storeId][$wonder]['top_billing_address_padding'] = explode(',', $this->getConfig('address_pad_billing', '0,0,0', false, $wonder, $storeId));
            $this->_packingsheet[$storeId][$wonder]['page_pad'] = explode(',', $this->getConfig('page_pad', '0,0,0', false, $wonder, $storeId));

            if ($this->_packingsheet[$storeId][$wonder]['pickpack_packlogo'] == 0 || $this->_packingsheet[$storeId][$wonder]['pickpack_logo_position'] != 'right')
                $this->_packingsheet[$storeId][$wonder]['pickpack_packbarcode'] = $this->getConfig('pickpack_packbarcode', 0, false, $wonder, $storeId);
            else $this->_packingsheet[$storeId][$wonder]['pickpack_packbarcode'] = 0;
            ...
            if ($this->_packingsheet[$storeId][$wonder]['filter_items_by_status'] == 2)
                $this->_packingsheet[$storeId][$wonder]['show_qty_options'] = $this->getConfig('show_qty_options_type_shiped_items', 1, false, $wonder, $storeId);
            else $this->_packingsheet[$storeId][$wonder]['show_qty_options'] = $this->getConfig('show_qty_options', 1, false, $wonder, $storeId);
            ...
            $this->_packingsheet[$storeId][$wonder]['product_sku_title'] = trim($this->getConfig('sku_title', '', false, $wonder, $storeId));
            ...
            //start Sales Igniter Rental custom column
            $this->_packingsheet[$storeId][$wonder]['show_sales_igniter_rental_date'] = $this->getConfig('show_sales_igniter_rental_date', 0, false, $wonder, $storeId);
            if ($this->_packingsheet[$storeId][$wonder]['show_sales_igniter_rental_date'] == 1){
                $this->_packingsheet[$storeId][$wonder]['show_sales_igniter_rental_date_title'] = trim($this->getConfig('show_sales_igniter_rental_date_title', 'Date', false, $wonder, $storeId));
                $this->_packingsheet[$storeId][$wonder]['show_sales_igniter_rental_date_xpos'] = $this->getConfig('show_sales_igniter_rental_date_xpos', 300, false, $wonder, $storeId);
            }
            //start numberlist
            ...
            if ($this->_packingsheet[$storeId][$wonder]['prices_yn']){
                $this->_packingsheet[$storeId][$wonder]['product_line_base_prices_yn'] = $this->getConfig('product_line_base_prices_yn', 0, false, $wonder, $storeId);
                $this->_packingsheet[$storeId][$wonder]['total_base_prices_yn'] = $this->getConfig('total_base_prices_yn', 0, false, $wonder, $storeId);
            }else{
                $this->_packingsheet[$storeId][$wonder]['product_line_base_prices_yn'] = 0;
                $this->_packingsheet[$storeId][$wonder]['total_base_prices_yn'] = 0;
            }
            ...
            if ($this->_packingsheet[$storeId][$wonder]['total_subtotal'] == 2)
            ...
            else $this->_packingsheet[$storeId][$wonder]['total_subtotal_with_tax_yn'] = 0;
            ...
            $this->_packingsheet[$storeId][$wonder]['pickpack_nudge_id_bottom_shipping_address'] = explode(",", $this->getConfig('pickpack_nudge_id_bottom_shipping_address', '0, 14',  true, $wonder, $storeId));
			$this->_packingsheet[$storeId][$wonder]['pickpack_nudge_id_bottom_shipping_address'][1] += self::NUDGE_BOTTOM_ORDER_ID;
            ...
            $this->_packingsheet[$storeId][$wonder]['bottom_barcode_nudge'] = explode(",", $this->getConfig('bottom_barcode_nudge', '0,0', false, $wonder, $storeId));
            ...
            //order comment
            $this->_packingsheet[$storeId][$wonder]['notes_yn'] = $this->getConfig('notes_yn', 0, false, $wonder, $storeId);
            $this->_packingsheet[$storeId][$wonder]['notes_title'] = trim($this->getConfig('notes_title', '', false, $wonder, $storeId));
            $this->_packingsheet[$storeId][$wonder]['notes_position'] = $this->getConfig('notes_position', 'yesshipping', false, $wonder, $storeId);
            $this->_packingsheet[$storeId][$wonder]['notes_filter_options'] = $this->getConfig('notes_filter_options', 'no', false, $wonder, $storeId);
            $this->_packingsheet[$storeId][$wonder]['notes_filter'] = explode("|",trim($this->getConfig('notes_filter', '', false, $wonder, $storeId)));
            $this->_packingsheet[$storeId][$wonder]['check_comments_for_gift_message_yn'] = $this->getConfig('check_comments_for_gift_message_yn', 0, false, $wonder, $storeId);
            /*************************** END MESSAGE CONFIG SECTION *******************************/

        }
        return $this->_packingsheet[$storeId][$wonder];
    }

    public function getAddressLabelConfigArray($storeId = null)
    {
        $wonder = 'label';
        if ($storeId === null) $storeId = Mage::app()->getStore()->getStoreId();
        if (!isset($this->_addressLabel[$storeId][$wonder])) {
            $this->_addressLabel[$storeId][$wonder]['use_courierrules_shipping_label'] = $this->getConfig('show_customs_declaration',0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['page_size'] = $this->getConfig('page_size','a4', false, $wonder, $storeId);
            if ($this->_addressLabel[$storeId][$wonder]['page_size'] == 'a4'){
                $this->_addressLabel[$storeId][$wonder]['label_width'] = $this->getConfig('label_width',190, false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['label_height'] = $this->getConfig('label_height',168, false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['paper_margin'] = explode(",", $this->getConfig('paper_margin', '22,30,22,30', false, $wonder, $storeId));
                $this->_addressLabel[$storeId][$wonder]['label_padding'] = explode(",", $this->getConfig('label_padding', '5,5,5,5', false, $wonder, $storeId));
                $this->_addressLabel[$storeId][$wonder]['nudge_shipping_address'] = explode(",", $this->getConfig('nudge_shipping_address', '0,0', false, $wonder, $storeId));
                $this->_addressLabel[$storeId][$wonder]['show_address_barcode_yn'] = $this->getConfig('show_address_barcode_yn', 0, false, $wonder, $storeId);
            }elseif ($this->_addressLabel[$storeId][$wonder]['page_size'] == 'letter'){
                $this->_addressLabel[$storeId][$wonder]['label_width'] = $this->getConfig('label_width_letter',190, false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['label_height'] = $this->getConfig('label_height_letter',168, false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['paper_margin'] = explode(",", $this->getConfig('paper_margin_letter', '22,30,22,30', false, $wonder, $storeId));
                $this->_addressLabel[$storeId][$wonder]['label_padding'] = explode(",", $this->getConfig('label_padding_letter', '5,5,5,5', false, $wonder, $storeId));
                $this->_addressLabel[$storeId][$wonder]['nudge_shipping_address'] = explode(",", $this->getConfig('nudge_shipping_address_letter', '0,0', false, $wonder, $storeId));
                $this->_addressLabel[$storeId][$wonder]['show_address_barcode_yn'] = $this->getConfig('show_address_barcode_yn_letter', 0, false, $wonder, $storeId);
            }
            $this->_addressLabel[$storeId][$wonder]['show_order_id_barcode_yn'] = $this->getConfig('show_order_id_barcode_yn',0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['nudge_order_id_barcode'] = explode(",", $this->getConfig('nudge_order_id_barcode', '0,0', true, $wonder, $storeId));
            $this->_addressLabel[$storeId][$wonder]['label_show_order_id_yn'] = $this->getConfig('label_show_order_id_yn',0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['nudge_order_id'] = explode(",", $this->getConfig('nudge_order_id', '0,0', true, $wonder, $storeId));
            $this->_addressLabel[$storeId][$wonder]['font_family_label'] = $this->getConfig('font_family_label', 'opensans', false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['font_style_label'] = $this->getConfig('font_style_label', 'regular', false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['font_size_label'] = $this->getConfig('font_size_label', 15, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['font_color_label'] = $this->getConfig('font_color_label', '#000000', false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['label_logo_yn2'] = $this->getConfig('label_logo_yn2', 0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['top_shipping_address_background_yn_scale'] = $this->getConfig('top_shipping_address_background_yn_scale', 0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['label_nudgelogo'] = explode(",", $this->getConfig('label_nudgelogo', '0,0', true, $wonder, $storeId));
            $this->_addressLabel[$storeId][$wonder]['label_return_address_yn'] = $this->getConfig('label_return_address_yn', 0, false, $wonder, $storeId);
            if ($this->_addressLabel[$storeId][$wonder]['label_return_address_yn'] == 1){
                $this->_addressLabel[$storeId][$wonder]['label_return_address'] = $this->getConfig('label_return_address', '', false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_family_return_label'] =$this->getConfig('font_family_return_label', 'opensans', false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_style_return_label'] = $this->getConfig('font_style_return_label', 'regular', false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_size_return_label'] = $this->getConfig('font_size_return_label', 9, false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_color_return_label'] = $this->getConfig('font_color_return_label', '#000000', false, $wonder, $storeId);
                if ($this->_addressLabel[$storeId][$wonder]['page_size'] == 'a4'){
                    $this->_addressLabel[$storeId][$wonder]['label_return_address_nudge'] = explode(",", $this->getConfig('nudge_return_label_a4', '0,0', true, $wonder, $storeId));
                }elseif ($this->_addressLabel[$storeId][$wonder]['page_size'] == 'letter'){
                    $this->_addressLabel[$storeId][$wonder]['label_return_address_nudge'] = explode(",", $this->getConfig('nudge_return_label_letter', '0,0', true, $wonder, $storeId));
                }
            }elseif ($this->_addressLabel[$storeId][$wonder]['label_return_address_yn'] == 'yesside'){
                $this->_addressLabel[$storeId][$wonder]['label_return_address'] = $this->getConfig('label_return_address_side', '', false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_family_return_label'] = $this->getConfig('font_family_return_label_side', 'opensans', false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_style_return_label'] = $this->getConfig('font_style_return_label_side', 'regular', false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_size_return_label'] = $this->getConfig('font_size_return_label_side', 9, false, $wonder, $storeId);
                $this->_addressLabel[$storeId][$wonder]['font_color_return_label'] = $this->getConfig('font_color_return_label_side', '#000000', false, $wonder, $storeId);
                if ($this->_addressLabel[$storeId][$wonder]['page_size'] == 'a4'){
                    $this->_addressLabel[$storeId][$wonder]['label_return_address_nudge'] = explode(",", $this->getConfig('nudge_return_label_side_a4', '0,0', true, $wonder, $storeId));
                }elseif ($this->_addressLabel[$storeId][$wonder]['page_size'] == 'letter'){
                    $this->_addressLabel[$storeId][$wonder]['label_return_address_nudge'] = explode(",", $this->getConfig('nudge_return_label_side_letter', '0,0', true, $wonder, $storeId));
                }
            }
            $this->_addressLabel[$storeId][$wonder]['show_product_list'] = $this->getConfig('show_product_list', 0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['rotate_product_list'] = $this->getConfig('rotate_product_list', 0, false, $wonder, $storeId);
            $this->_addressLabel[$storeId][$wonder]['nudge_rotate_product_list'] = explode(",", $this->getConfig('nudge_rotate_product_list', '0,0', true, $wonder, $storeId));
        }
        return $this->_addressLabel[$storeId][$wonder];
    }
    ...
    public function getZebraLabelConfigArray($storeId = null)
    {
        ...
        if(!isset($this->_zebralabel[$storeId])) {
            ...
            $this->_zebralabel[$storeId]['nudge_dimension_zebra'] = explode(",", $this->getConfig('nudge_dimension_zebra', '432,288', false, 'label_zebra',$storeId));
            ...
            //zipcode barcode
            $this->_zebralabel[$storeId]['show_zipcode_barcode'] = $this->getConfig('show_zipcode_barcode', 0, false, 'label_zebra',$storeId);
            $this->_zebralabel[$storeId]['zipcode_barcode_fontsize'] = $this->getConfig('zipcode_barcode_fontsize', 16, false, 'label_zebra', $storeId);
            $this->_zebralabel[$storeId]['zipcode_barcode_nudge'] = explode(",", $this->getConfig('zipcode_barcode_nudge', '0,0', true, 'label_zebra', $storeId));
            //order id barcode
            $this->_zebralabel[$storeId]['show_order_id_barcode_yn'] = $this->getConfig('show_order_id_barcode_yn', 1, false, 'label_zebra',$storeId);
            $this->_zebralabel[$storeId]['order_id_barcode_font_size'] = $this->getConfig('order_id_barcode_font_size', 16, false, 'label_zebra', $storeId);
            ...
            $this->_zebralabel[$storeId]['tracking_number_barcode_yn'] = $this->getConfig('tracking_number_barcode_yn', 1, false, 'label_zebra',$storeId);
            $this->_zebralabel[$storeId]['tracking_number_barcode_font_size'] = $this->getConfig('tracking_number_barcode_font_size', 16, false, 'label_zebra', $storeId);
            $this->_zebralabel[$storeId]['tracking_number_barcode_nudge'] = explode(",", $this->getConfig('tracking_number_barcode_nudge', '0,0', true, 'label_zebra', $storeId));
            ...
            /******************* SHIPPING ADDRESS *******************/
            $this->_zebralabel[$storeId]['font_style_label'] = $this->getConfig('font_style_label', self::FONT_STYLE_LABEL, false, 'label_zebra', $storeId);
            $this->_zebralabel[$storeId]['font_family_label'] = $this->getConfig('font_family_label', self::FONT_FAMILY_LABEL, false, 'label_zebra', $storeId);
            $this->_zebralabel[$storeId]['font_color_label'] = $this->validate_html_color($this->getConfig('font_color_label', self::FONT_COLOR_LABEL, false, 'label_zebra', $storeId));
            $this->_zebralabel[$storeId]['font_size_label'] = $this->getConfig('font_size_label', self::FONT_SIZE_LABEL, false, 'label_zebra', $storeId);
            ...
            if ($this->_zebralabel[$storeId]['show_product_list']){
				...
				$this->_zebralabel[$storeId]['show_product_qty'] = 0;
          
		    //product name
            if ($this->_zebralabel[$storeId]['show_product_list']){
                $this->_zebralabel[$storeId]['show_product_name'] = $this->getConfig('show_product_name', 1, false, 'label_zebra',$storeId);
                $this->_zebralabel[$storeId]['product_name_title'] = $this->getConfig('product_name_title', '', false, 'label_zebra',$storeId);
            ...
            if ($this->_zebralabel[$storeId]['show_product_list']){
				...
				$this->_zebralabel[$storeId]['show_product_sku'] = 0;
            ...
            if ($this->_zebralabel[$storeId]['show_product_list']){
                ...
                $this->_zebralabel[$storeId]['product_barcode_title'] = $this->getConfig('product_barcode_title', '', false, 'label_zebra',$storeId);
                $this->_zebralabel[$storeId]['product_barcode_font_size'] = $this->getConfig('product_barcode_font_size', 10, false, 'label_zebra',$storeId);
                $this->_zebralabel[$storeId]['product_barcode_xpos'] = $this->getConfig('product_barcode_xpos', 30, false, 'label_zebra',$storeId);
            }else
				$this->_zebralabel[$storeId]['show_product_barcode'] = 0;
    ...
    public function getConfig($field, $default = '', $add_default = true, $group = 'wonder', $store = null, $trim = true,$section = 'pickpack_options') {
        ...
        if(!isset($this->_config[$key])) {
            ...
            if (!isset($value) || is_null($value) || ($value == '')) {
                ...
                if ((bool)strtotime(date(trim($value))))
                    $value = $value;
                else $value = $default;
