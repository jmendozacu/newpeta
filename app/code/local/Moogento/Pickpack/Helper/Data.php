...
class Moogento_Pickpack_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getWidthHeightImage($product_id, $product_images_source_res, $product_images_maxdimensions) {
		...
		if (isset($image_obj)) {
			...
			$image_dimension[0] = $img_width;
			$image_dimension[1] = $img_height;
		}

        $this->_imageWidthHeightCache[$product_images_source_res][$image_product->getId()] = $image_dimension;
	...
	public function getImagePaths($product, $product_images_source, $product_images_maxdimensions) {
        ...
        if(isset($image_galleries['images']) && count($image_galleries['images']) > 0) {
			...
			$img_dimension = $this->getWidthHeightImage($image_product, $product_images_source_res, $product_images_maxdimensions);
			if(is_array($img_dimension) && count($img_dimension) > 1) {
				$img_width = $img_dimension[0];
				$img_height = $img_dimension[1];
			}
