...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Labelzebra_Products extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    public function caculateProductElementArrayXpos(){
        ...
        if(isset($product_line_array_xpos))
			asort($product_line_array_xpos);
		else
			$product_line_array_xpos = null;
    ...
    public function showProductSkuBarcode(){
        if ($this->zebralabelConfig['show_product_barcode']){
            ...
            //check need to create new page
            if ($this->zebralabelConfig['product_barcode_in_separate_line']){
                $barcode_height = $this->y - (1.5 * $this->zebralabelConfig['product_barcode_font_size']);
                if ($barcode_height <= $this->zebralabelConfig['label_padding_zebra'][2]){
                    $pdf->newPageZebra();
                    $this->y = $pdf->y;
                }
            }
            ...
            if ($this->zebralabelConfig['product_barcode_in_separate_line']){
                ...
                $sku_barcodeY = $this->y - (1.5 * $this->zebralabelConfig['product_barcode_font_size']);
                ...
                $sku_barcodeY = $this->y;
                $after_print_barcode_y = $this->y - (1.1 * $this->zebralabelConfig['product_barcode_font_size']);
            }
            ...
            $this->printProductBarcode($page,$barcode,$barcode_type,$this->zebralabelConfig['show_product_barcode'],$this->zebralabelConfig['product_barcode_xpos'],$sku_barcodeY,$padded_right,$font_family_barcode,$this->zebralabelConfig['product_barcode_font_size'],$white_color);
    ...
    public function printProductBarcode($page,$barcode,$barcode_type,$product_sku_barcode_yn,$sku_barcodeX,$sku_barcodeY,$padded_right,$font_family_barcode,$barcode_font_size,$white_color) {
        ...
        $barcodeWidth = $this->parseString($barcodeString, Zend_Pdf_Font::fontWithPath($this->action_path . $font_family_barcode), $barcode_font_size);
        ...
        if (($sku_barcodeX + $barcodeWidth) > $padded_right){
            ...
            $pdf->drawText($page,"!! TRIMMED BARCODE !!", ($sku_barcodeX), ($sku_barcodeY));
        }
        else if ( ($sku_barcodeX + $barcodeWidth) >= $nextCollumnX)
            $pdf->drawText($page,$barcodeString, ($sku_barcodeX), ($sku_barcodeY - (1.3*$barcode_font_size)), 'CP1252');
        else
            $pdf->drawText($page,$barcodeString, ($sku_barcodeX), ($sku_barcodeY), 'CP1252');
