...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Storeaddress extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    const COMPANY_ADDRESS_NUDGE_X = 80;
    const COMPANY_ADDRESS_NUDGE_Y = 0;
    ...
    public function showAddress()
    {
        ...
        if ($company_address_yn == 'yesgroup' && Mage::helper('pickpack')->isInstalled('Moogento_CourierRules') && is_object($order->getShippingAddress())) {
            $zoneIds = array(
                'group1' => $this->_getConfig('pickpack_company_address_group1_zone', '', false, $wonder, $storeId),
                'group2' => $this->_getConfig('pickpack_company_address_group2_zone', '', false, $wonder, $storeId),
                'group3' => $this->_getConfig('pickpack_company_address_group3_zone', '', false, $wonder, $storeId),
            );
            ...
            foreach ($zoneIds as $code => $zoneId) {
                if ($zoneId) {
                    ...
                    if ($zone->validate($order))
                        $company_address = $this->_getConfig('pickpack_company_address_' . $code, '', false, $wonder, $storeId);
                }
            }
        }
        ...
        if ($company_address != '') {
            ...
            $company_address_nudge[0] += self::COMPANY_ADDRESS_NUDGE_X;
            $company_address_nudge[1] += self::COMPANY_ADDRESS_NUDGE_Y;
            ...
            if (isset($headerLogo) && ($company_vert_line === true) && (strtoupper($background_color_subtitles) != '#FFFFFF')) {
                if ($headerLogo->getLogoPosition() == 'left') {
                ...
                } elseif ($headerLogo->getLogoPosition() == 'right') {
