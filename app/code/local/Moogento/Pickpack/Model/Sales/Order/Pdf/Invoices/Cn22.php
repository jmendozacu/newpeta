...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Cn22 extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    public function nooPage($rotate_document = 0,$page_size = '',$store_id=0) {
        ...
        elseif ($page_size == 'a5-landscape') {
        	...
        	$dimension = explode(",", $this->_getConfig('page_size_custom', '279,245', false,'cn22_label',$store_id,true,'cn22_options'));        
        	$width_pt = $dimension[0];
        	$height_pt = $dimension[1];    
...
//				$print_column_count =1;
			...
			try{
				$customs_section_model->printCustomsSection(0,$page,$order,'cn22',$current_label_xpos,$current_label_ypos,$item_qty_array,$label_width,$label_height, null, 'check_limit_resize');

                if($current_label_xpos + ($label_width+$label_nudge_right+$label_nudge_left)*2 <= $this->padded_right)
