...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Address extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    const BOTTOM_LABEL_ORDERID_DEFAULT_NUDGE_Y = -5;
    ...
    const BOTTOM_BARCODE_DEFAULT_NUDGE_Y = 0;
    const BOTTOM_TRACKING_DEFAULT_NUDGE_X = 0;
    const BOTTOM_TRACKING_DEFAULT_NUDGE_Y = 16;
	const TOP_ADDRESS_DEFAULT_PADDING_TOP = 10;
	const TOP_ADDRESS_DEFAULT_PADDING_BOTTOM = 10;
    ...
    public function showAddressDetails() {
		...
		$address_pad[0] += self::TOP_ADDRESS_DEFAULT_PADDING_TOP;
		$address_pad[1] += self::TOP_ADDRESS_DEFAULT_PADDING_BOTTOM;
        ...
        if ($shipment_details_yn == 1) {
                ...
                if ($payment_order) {
                    if(Mage::getDesign()->getArea() == 'frontend'){
                        $paymentInfo = Mage::helper('payment')->getInfoBlock($payment_order)
                            ...
                            ->setTemplate('../../../../adminhtml/default/default/template/payment/info/pdf/default.phtml')
                            ->toHtml();
                    }
                    else {
                        $paymentInfo = Mage::helper('payment')->getInfoBlock($payment_order)
            ...
            if ($shipment_details_customer_id > 0 && $customer_id != '') {
                if($generalConfig['shipment_details_bold_label_yn'] == 1)
                    $this->_setFont($page, "bold", $generalConfig['font_size_body'], $generalConfig['font_family_body'], $generalConfig['non_standard_characters'], $generalConfig['font_color_body']);
                ...
                $this->_setFont($page, $generalConfig['font_style_body'], $generalConfig['font_size_body'], $generalConfig['font_family_body'], $generalConfig['non_standard_characters'], $generalConfig['font_color_body']);
                if ($shipment_details_customer_id == 2) {
                   $customer_id = $this->getCustomerIncrementId($order);
                }
    ...
    public function showShippingAddressBarcode() {
        ...
        $fontPath = $this->action_path . $generalConfig['font_family_barcode'];
        $barcodeSize = Mage::helper('pickpack/barcode')->getBarcodeSizeOnRealFont($barcodeString, $fontPath, $barcode_font_size);


        $x = ($addressFooterXY[0] + $left_down + $bottom_barcode_nudge[0]);
        $y = ($addressFooterXY[1] + $barcode_font_size - 12 - $left_down + $bottom_barcode_nudge[1] + 5 );

        $page->setFillColor($white_color);
        $page->setLineColor($white_color);
        $page->drawRectangle($x - 3, $y - 3, $x + $barcodeSize['width'] + 3, $y + $barcodeSize['height'] + 3);

        $page->setFillColor(new Zend_Pdf_Color_Rgb(0, 0, 0));
        $page->setFont(Zend_Pdf_Font::fontWithPath($this->action_path . $generalConfig['font_family_barcode']), $barcode_font_size);
        $this->_drawText($barcodeString, $x, $y, 'CP1252');
    ...
    public function showAddress()
    {
        ...
        if ($billing_details_yn == 1) {
            ...
            if($billing_phone_yn == 1 && strlen(trim($billing_phone)) > 5)
        ...
        $temp_bottom_y_right_column = $this->address_top_y;
        foreach ($cycle_address_array as $i => $value) {
            ...
            if ($shipping_billing_title_position != 'beside') {
                ...
                if (($billing_details_yn == 1) && isset($show_this_billing_line[$i])) {
                    ...
                    if ($line_bold_billing == 1) {
                        ...
                        $this->_setFont($page, $generalConfig['font_style_body'], ($generalConfig['font_size_body']), $generalConfig['font_family_body'], $generalConfig['non_standard_characters'], $generalConfig['font_color_body']);
                    if ($billing_title != '') {
                        $address_billing_line_y = ($this->address_top_y - $addon_billing_y_updown_title - ($this->line_height_top * $this->i_space) - $this->line_addon - $address_pad_billing[0]);
                        $this->_drawText($value_billing, ($address_right_x + $address_pad_billing[2]), $address_billing_line_y);
                        if ($temp_bottom_y_right_column > $address_billing_line_y)
                            $temp_bottom_y_right_column = $address_billing_line_y;
                    } else{
                        $address_billing_line_y = (($this->address_top_y) - $addon_billing_y_updown_title - ($this->line_height_top * $this->i_space) - $this->line_addon - $address_pad_billing[0]);
                        $this->_drawText($value_billing, ($address_right_x + $address_pad_billing[2]), $address_billing_line_y);
                        if ($temp_bottom_y_right_column > $address_billing_line_y)
            ...
            if ($this->top_y_right_column > $temp_bottom_y_right_column)
                $this->top_y_right_column = $temp_bottom_y_right_column - 90;
            /**** END PRINTING TOP BILLING AND SHIPPING ****/
        }

        $this->top_y_right_column = $this->top_y_right_column - $this->packingsheetConfig['top_billing_address_padding'][1];
        ...
        foreach ($cycle_address_array as $i => $value) {
            ...
            if (($this->packingsheetConfig['pickpack_bottom_shipping_address_yn'] == 1) && isset($show_this_shipping_line_bottom[$i])) {
                ...
                if ($this->packingsheetConfig['capitalize_label2_yn'] == 1 || $this->packingsheetConfig['capitalize_label2_yn'] == 2) {
                    if (strtolower($customer_country) == 'united states')
						$value = preg_replace('~,$~', '', $value);
                    $font_size_adjust = 2;
                }
    ...
    private function drawTrackingNumber() {
        $order = $this->getOrder();
        $tracking_number = $this->getTrackingNumber($order);
        ...
        if($tracking_number != '') {
	        ...
	        $addressFooterXY[1] += self::BOTTOM_LABEL_ADDRESS_DEFAULT_NUDGE_Y;
			$white_color = Mage::helper('pickpack/config_color')->getPdfColor('white_color');
            $tracking_number_fontsize = $this->_getConfig('tracking_number_fontsize', 10, false, $this->getWonder(), $this->getStoreId());
			$tracking_number_nudge = explode(",", $this->_getConfig('tracking_number_nudge', '0,0', false, $this->getWonder(), $this->getStoreId()));
	        $tracking_number_nudge[0] += self::BOTTOM_TRACKING_DEFAULT_NUDGE_X;
	        $tracking_number_nudge[1] += self::BOTTOM_TRACKING_DEFAULT_NUDGE_Y;			
            $this->_setFont($page, 'semibolditalic', ($tracking_number_fontsize + 0.5), $generalConfig['font_family_body'], $generalConfig['non_standard_characters'], $generalConfig['font_color_body']);

            $this->_drawText($tracking_number, ($addressFooterXY[0] + $tracking_number_nudge[0]), ($addressFooterXY[1] + $tracking_number_nudge[1] - $tracking_number_fontsize), 'CP1252');
		}
    }
    ...
    public function  getPageFooterHeight(){
        ...
        if($this->_getConfig('show_customs_declaration',0, false, $wonder, $storeId)) {
            ...
            if (
                $show_customs_declaration_nudge[1] < $this->generalConfig['page_size'][1] / 2
                &&
                $show_customs_declaration_nudge[1] > $footerY
            )
                $footerY = $show_customs_declaration_nudge[1];
        }
    ...
    private function getCustomerIncrementId($order)
    {
    ...
    protected function _drawText($text, $x, $y, $charEncoding = 'UTF-8') {
        ...
        /* Retrieve dummy value Y/N*/
        $dummyFilteringYn = (int)$this->_getConfig('dummy_values_yn', 0, false, 'general');
        /* Retrieve dummy word*/
        $dummyStopWord = (string)trim($this->_getConfig('dummy_values_text', null, false, 'general'));

        if (1 == $dummyFilteringYn && $dummyStopWord !== '') {
            if (false !== (bool)strpos($text, $dummyStopWord)) {
                 return;
            }
        }
