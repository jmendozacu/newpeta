...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Ordernote extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    public function showBottomNote() {
        ...
        $notes_filter_options = $this->_getConfig('notes_filter_options', '', false, $wonder, $storeId);
        $notes_filter = trim(strtolower($this->_getConfig('notes_filter', '', false, $wonder, $storeId)));
        $positional_message_box_fixed_position_dimension_x = $this->_getConfig('positional_message_box_fixed_position_dimension', 250, false, $wonder, $storeId);
        ...
        $message_character_breakpoint = round(($positional_message_box_fixed_position_dimension_x / $pt_per_char));
        ...
        if ($orderComments) {
            ...
            if ($notes_position == 'yesbox') {
                $max_name_length = $positional_message_box_fixed_position_dimension_x - 10;
                $msgX = $pageConfig['padded_left'] + $positional_message_box_fixed_position[0];
                $right_bg_gift_msg = $msgX + $positional_message_box_fixed_position_dimension_x;
    ...
    public function showAitocOrderComments(){
        $page = $this->getPage();
        $order = $this->getOrder();
        //need re.caculate this pageConfig value
        $pageConfig = $this->getPageConfig();

        $aitoc_aitcheckoutfields_element = Mage::getModel('pickpack/sales_order_pdf_invoices_elements_module_aitoc_aitcheckoutfields', array($this, $order));
        $Comments = $aitoc_aitcheckoutfields_element->getAitocOrderComments();

        $witdh_comment = ($pageConfig['padded_right'] - 20) - ($pageConfig['padded_left'] + 20);
        $comment_array = array();
        foreach ($Comments as $comment){
            $comment_line = $comment['value'];
            $array_line = $this->wordWrapOnRealFont($comment_line, $this->generalConfig['font_family_message'], $this->generalConfig['font_style_message'], $this->generalConfig['font_size_message'], $witdh_comment);
            $comment_array = array_merge($comment_array, $array_line);
        }

        if (count($comment_array)){
            $this->flag_message_after_shipping_address = 1;
            /****************************** PRINT ORDER COMMENT BOX ********************************/

            $commments_block_height = ((count($comment_array) + 1) * 1.2 * $this->generalConfig['font_size_message']) + (1.2 * $this->generalConfig['font_size_message']);

            $this->subheader_start -= 20;
            $x1_bg = $pageConfig['padded_left'];
            $y1_bg = $this->subheader_start;
            $x2_bg = $pageConfig['padded_right'];
            $y2_bg = $y1_bg - $commments_block_height;

            $background_color = new Zend_Pdf_Color_Html($this->generalConfig['bkg_color_message']);

            $this->printBackGroundBox($x1_bg, $y1_bg ,$x2_bg ,$y2_bg ,$this->generalConfig['fill_bkg_color_comments'], $background_color);

            $xpos_comment = $x1_bg + 20;
            $ypos_comment = $y1_bg - 1.4 * $this->generalConfig['font_size_message'];
            ...
            $page->drawText($this->packingsheetConfig['notes_title'].":", $xpos_comment, $ypos_comment, 'UTF-8');
            $ypos_comment = $ypos_comment - 1.2 * $this->generalConfig['font_size_message'];

            $this->_setFont($page, $this->generalConfig['font_style_message'], $this->generalConfig['font_size_message'], $this->generalConfig['font_family_message'], $this->generalConfig['non_standard_characters'], $this->generalConfig['font_color_message']);
            foreach ($comment_array as $line){
                $page->drawText($line, $xpos_comment, $ypos_comment, 'UTF-8');
                $ypos_comment = $ypos_comment - 1.2 * $this->generalConfig['font_size_message'];
            }

            $this->subheader_start = $ypos_comment;
        }
    }

    public function printBackGroundBox($x1, $y1, $x2, $y2, $fill = 0, $background_color){
        $page = $this->getPage();

        $fill = 1;

        $page->setFillColor($background_color);
        $page->setLineColor($background_color);
        $page->setLineWidth(0.5);
        if ($fill == 0) {
            $page->drawLine($x1, $y1, $x2, $y1);//top line
            $page->drawLine($x1, $y2, $x2, $y2);//bottom line
            $page->drawLine($x1, $y1, $x1, $y2);//left line
            $page->drawLine($x2, $y1, $x2, $y2);//right line
        }
        else{
