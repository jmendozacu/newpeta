...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Separated extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
	...
	const PRODUCT_IMAGE_NUDGE_X = 26;
	const PRODUCT_IMAGE_NUDGE_Y = 12;
	...
	*/
    private function writeSummary($label, $value, $page, $key_symbol = 'square'){
        ...
        /* column alignment */
        $posValueX = (substr($value,1,1) != ' ') ? 500 : 500 - ($generalConfig['font_size_body'] * 1.2);
        $page->drawText($value, $posValueX, ($this->y-0.5), 'UTF-8');
...
{if country}{country}|{/if}";
        ...
        if($address_label_yn == 1){
            ...
            $dimension_label_address = explode(",", $this->_getConfig('label_dimension_fields', '250,200', false, 'picks'));
            $label_width     = $dimension_label_address[0];
            $label_height    = $dimension_label_address[1];
        ...
        /* total section */
        $showTotalWeight = $this->_getConfig('pickpack_show_total_weight_yn_separated', 0, false, 'picks');
        $showTotalShipping = $this->_getConfig('pickpack_show_total_shipping_yn_separated', 0, false, 'picks');
        $showGrandTotal = $this->_getConfig('pickpack_show_grandtotal_yn_separated', 0, false, 'picks');
        $total_weight = $total_shipping = $grand_total = 0;
        ...
        if($show_column_title == 1){
            ...
            $col_title_warehouse  = $this->_getConfig('col_title_position_warehouse', 'Warehouse', false, 'picks');
            $col_title_tickbox       = $this->_getConfig('col_title_position_tickbox', '', false, 'picks');
            $col_title_tickbox2       = $this->_getConfig('col_title_position_tickbox2', '', false, 'picks');
            $col_title_attr1       = $this->_getConfig('col_title_position_attr1', '', false, 'picks');
            $col_title_attr2      = $this->_getConfig('col_title_position_attr2', '', false, 'picks');
            $col_title_attr3       = $this->_getConfig('col_title_position_attr3', '', false, 'picks');
            $col_title_attr4       = $this->_getConfig('col_title_position_attr4', '', false, 'picks');
            $col_title_product_barcode     = $this->_getConfig('col_product_barcode', 'Barcode', false, 'picks');
	        $attr1_yn = $this->_getConfig('pickpack_shelving_yn_separated', 0, false, 'picks');
	        $attr2_yn = $this->_getConfig('pickpack_shelving2_yn_separated', 0, false, 'picks');
	        $attr3_yn = $this->_getConfig('pickpack_shelving3_yn_separated', 0, false, 'picks');
	        $attr4_yn = $this->_getConfig('pickpack_shelving4_yn_separated', 0, false, 'picks');
        }
	   ...
	   $shelving4_attribute = null;
        ...
        $product_images_yn = $this->_getConfig('product_images_yn', 0, false, 'picks');
        /* config of the product price for order-separated*/
        $priceyn = $this->_getConfig('pickpack_price_yn_separated', 0, false, 'picks');
        if($priceyn)
            $pricenudgeX = $this->_getConfig('pickpack_price_nudge_separated', 350, false, 'picks');
        ...
        if ($product_images_yn == 1) {
            ...
            $product_images_nudge = explode(',', trim($this->_getConfig('product_images_nudge', '0,0', false, $config_group)));
        ...
        if($product_images_yn == 1) {
            ...
            $columns_xpos_array['Image'] = isset($product_images_nudge[0]) ? $product_images_nudge[0] : null;
        }
        ...
        $orderDate = Mage::getStoreConfig('pickpack_options/picks/pickpack_orderdate');
        ...
        /*config of the order comments for separated orders*/
        $notes_yn_separated = $this->_getConfig('notes_yn_separated', 0, false, 'picks');
        /*end config of the order comments for separated orders*/
        ...
        foreach ($orders as $orderSingle) {
            ...
            //order comments
            if($notes_yn_separated){
                if($order->getStatusHistoryCollection(true)){
                    $orderComments[$order->getIncrementId()] = $order->getStatusHistoryCollection(true);
                }
            }
            ...
            foreach ($itemsCollection as $item) {
                ...
                if (($product_images_yn == 1) && !isset($sku_image_paths[$sku]['path'][0]))  {
                    ...
                    $img_dimension = $helper->getWidthHeightImage($product_id, $product_images_source_res, $product_images_maxdimensions);
                    if(is_array($img_dimension) && count($img_dimension) > 1){
                        $sku_image_paths[$sku]['width'] = $img_dimension[0];
                        $sku_image_paths[$sku]['height'] = $img_dimension[1];
                    }
                ...
                if ( 1 === (int)$orderDate) {
                    $date_format = Mage::getStoreConfig('pickpack_options/general/date_format');
                    $order_date_print = date($date_format, strtotime($order->getCreatedAt()));
                    $order_datetime[$order_id] = $order_date_print;
                    $font_temp = Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA);
                    $font_size_compare = ($generalConfig['font_size_subtitles'] - 2);
                    $line_width = $this->parseString($order_datetime[$order_id], $font_temp, $font_size_compare); // bigger = left
                }
                if ($warehouseyn == 1) {
                    if(Mage::helper('pickpack')->isInstalled('Innoexts_Warehouse'))
                    {
                ...
                if (isset($sku_order_id_qty[$order_id][$sku])) {
                    ...
                    {
                        /*Temp variables*/
                        $skuOrderIdQty  = $sku_order_id_qty[$order_id][$sku];
                        $skuOrderIdSQty = $sku_order_id_sqty[$order_id][$sku];

                        $sku_order_id_qty[$order_id][$sku] = ($skuOrderIdQty + $qty);
                        $sku_order_id_sqty[$order_id][$sku] = ($skuOrderIdSQty + $sqty);
                        ...
                        $product_build[$order_id][$sku]['qty'] = ($skuOrderIdQty + $qty);
                        $product_build[$order_id][$sku]['sqty'] = ($skuOrderIdSQty + $sqty);
                    ...
                    $each_order_per_page =  $this->_getConfig('pickpack_newpage_per_order',0, false,'picks', $store_id_arr[$order_id]);
                        ...
                        if ($supplier_skip_order == FALSE) {
                            ...
                            if($show_column_title == 1){
							    ...
							    if (isset($attr1_yn) && ($attr1_yn == 1))
                                    $page->drawText(Mage::helper('sales')->__($col_title_attr1), intval($shelvingX), $this->y, 'UTF-8');

							    if (isset($attr2_yn) && ($attr2_yn == 1))
                                    $page->drawText(Mage::helper('sales')->__($col_title_attr2), intval($shelving2X), $this->y, 'UTF-8');

							    if (isset($attr3_yn) && ($attr3_yn == 1))
                                    $page->drawText(Mage::helper('sales')->__($col_title_attr3), intval($shelving3X), $this->y, 'UTF-8');

							    if (isset($attr4_yn) && ($attr4_yn == 1))
                                    $page->drawText(Mage::helper('sales')->__($col_title_attr4), intval($shelving4X), $this->y, 'UTF-8');
       ...
       /* split bundle option */                    
        ...
        else 
        {
            ...
            foreach ($product_build as $order_id => $order_build) {
                ...
                if (1 === (int)$orderDate) {
                    $leftOffset = Mage::getStoreConfig('pickpack_options/picks/pickpack_orderdate_left_offset');
                    if ($barcodes == 1) $order_date_x = (2*$barcodeWidth + 110) + $leftOffset;
                    if ($shmethod == 1 && $barcodes == 1) $order_date_x = $leftOffset + $shipping_method_x + $barcodeWidth;
                    if ($shmethod == 0 && $barcodes == 0) $order_date_x = $leftOffset;
                    
                    $this->_setFont($page, 'bold', ($generalConfig['font_size_subtitles'] - 2), $generalConfig['font_family_subtitles'], $generalConfig['non_standard_characters'], $generalConfig['font_color_subtitles']);
                    $page->drawText($order_datetime[$order_id], $order_date_x, ($this->y - 15), 'UTF-8');
                }
                ...
                /* order comments*/
                if($notes_yn_separated && $orderComments){
                    $this->y = $this->printOrderNote($page,$tickbox_X,$padded_right,$orderComments[$order_id]);
                }
                // order gift message.
                if($giftmessage_yn == 1){
...
//                     $dsku = $value['sku'];
                   ...
                   $page->drawRectangle($tickbox2_X, ($this->y), $tickbox2_X + 7, ($this->y + 7));
                    ...
                    if ($product_qty_upsize_yn == 1 && $qty_string > 1 && $show_bundle_parent) {
                        if ($product_qty_rectangle == 1) {
                            ...
                            $page->setFillColor($black_color);
                            if (($qty_string >= 100) || (strlen($qty_string) >= 3)) {
                                $page->drawRectangle(($qtyX), ($this->y - 1), (($qtyX - 10) + (Mage::helper('core/string')->strlen($qty_string) * $generalConfig['font_size_body'])), ($this->y - 4 + $generalConfig['font_size_body'] * 1.2));
                            } else {
                                if (($qty_string >= 10) || (strlen($qty_string) >= 2)) {
                                    $page->drawRectangle(($qtyX - 1), ($this->y - 1), ($qtyX - 8 + (Mage::helper('core/string')->strlen($qty_string) * $generalConfig['font_size_body'])), ($this->y - 4 + $generalConfig['font_size_body'] * 1.2));
                                } else {
                                    $page->drawRectangle(($qtyX - 1), ($this->y - 1), ($qtyX - 2 + (Mage::helper('core/string')->strlen($qty_string) * $generalConfig['font_size_body'])), ($this->y - 4 + $generalConfig['font_size_body'] * 1.2));
                                }
                    ...
                    /* print product price */
                    if($priceyn == 1 && $show_bundle_parent){
                        ...
                        $draw_price = $orderSymbolCode . ' ' . $product_price;
                        $page->drawText($draw_price,intval($pricenudgeX), $this->y,'UTF-8');
                    }
                    ...
                    if (($product_images_yn == 1) && isset($sku_image_paths[$sku]['path'][0]) && $show_bundle_parent) {
                        ...
                        if (isset($sku_master_runcount) && $sku_master_runcount == 1)
							$this->y += ($product_images_line_nudge);

                        $image_x_addon = 0;
                        $image_x_addon_2 = 0;
	
                        $x1 = $product_images_nudge[0] + self::PRODUCT_IMAGE_NUDGE_X; 
                        $y1 = ($this->y - $sku_image_paths[$sku]['height'] + self::PRODUCT_IMAGE_NUDGE_Y);
                        $x2 = ($product_images_nudge[0] + $sku_image_paths[$sku]['width'] + self::PRODUCT_IMAGE_NUDGE_X);
                        $y2 = $this->y + self::PRODUCT_IMAGE_NUDGE_Y;
                        ...
                        $y1 = ($this->y - $sku_image_paths[$sku]['height'] - 5 + self::PRODUCT_IMAGE_NUDGE_Y) + $product_images_nudge[1];
                        $y2 = ($this->y - 5 + self::PRODUCT_IMAGE_NUDGE_Y)+ $product_images_nudge[1];
                    ...
                    if (isset($sku_bundle[$order_id][$sku]) && is_array($sku_bundle[$order_id][$sku])) {
                        ...
                        foreach ($sku_bundle[$order_id][$sku] as $key => $value) {
                            ...
                            if($priceyn == 1){
                                $orderSymbolCode = Mage::app()->getLocale()->currency($order->getOrderCurrencyCode())->getSymbol();
                                $product_price = number_format($product_child->getPrice(), 2, '.', ',');
                                $draw_price = $orderSymbolCode . ' ' . $product_price;
                                $page->drawText($draw_price,intval($pricenudgeX), $this->y,'UTF-8');
                            }
                          ...
                          /************/
                            ...
                            if (($product_images_yn == 1) && isset($sku_image_paths[$sku]['path'][0])) {
                                ...
                                $image_x_addon = 0;
                                $image_x_addon_2 = 0;
                                $x1 = $product_images_nudge[1];
                                $y1 = ($this->y - $sku_image_paths[$sku]['height'] );
                                $x2 = ($product_images_nudge[1] + $sku_image_paths[$sku]['width']);
                                $y2 = ($this->y);
            ...
            if (($showcount_yn == 1) || ($showcost_yn == 1) || ($order_count_yn == 1) || ($showTotalWeight == 1 && $total_weight > 0) ||
                ($showTotalShipping == 1 && $total_shipping > 0) || ($showGrandTotal == 1 && $grand_total > 0)) {
                if ($page_count == 1)
                    $this->y = $thisYbase;
                ...
                if($showTotalWeight == 1 && $total_weight > 0)
                    $shipYbox += $generalConfig['font_size_body'];

                if($showTotalShipping == 1 && $total_shipping > 0)
                    $shipYbox += $generalConfig['font_size_body'];

                if($showGrandTotal == 1 && $grand_total > 0)
                    $shipYbox += $generalConfig['font_size_body'];
            ...
            if($showTotalWeight == 1 && $total_weight > 0)
                $this->writeSummary($helper->__('Total Weight'), number_format($total_weight, 2, '.', ',') . ' kg', $page, 'none');

            if($showTotalShipping == 1 && $total_shipping > 0)
                $this->writeSummary($helper->__('Total Shipping'), $currency_symbol . '  ' . number_format($total_shipping, 2, '.', ','), $page, 'none');

            if($showGrandTotal == 1 && $grand_total > 0)
                $this->writeSummary($helper->__('Grand Total'), $currency_symbol . '  ' . number_format($grand_total, 2, '.', ','), $page, 'none');
    ...
    public function printOrderNote($page,$padded_left,$padded_right,$orderComments){
        $storeId = Mage::app()->getStore()->getId();
        $generalConfig = $this->getGeneralConfig();

        $notes_title = $this->_getConfig('notes_title_separated', 'Order Comments', false, 'picks',$storeId);
        $notes_filter_options = $this->_getConfig('notes_filter_options_separated', 'yesfrontend', false, 'picks',$storeId);
        if($notes_filter_options == 'yestext'){
            $notes_filter = $this->_getConfig('notes_filter_separated', '', false, 'picks',$storeId);
            $notes_filter = preg_replace('/^([\'"])(.*)\\1$/', '\\2', $notes_filter);
        }
        $check_comments_for_gift_message_yn_separated = $this->_getConfig('check_comments_for_gift_message_yn_separated', 0, false, 'picks',$storeId);
        ...
        if ($orderComments) {
            ...
            $max_name_length = $padded_right - 10;
            $msgX = $padded_left;
            ...
            foreach ($orderComments as $comment) {
                ...
                if (Mage::helper('pickpack')->isInstalled('Ess_M2ePro') && $check_comments_for_gift_message_yn_separated) {
                    $check_comments_for_gift_message_filter = $this->_getConfig('check_comments_for_gift_message_filter_separated', 'Checkout Message', false, 'picks', $storeId);
