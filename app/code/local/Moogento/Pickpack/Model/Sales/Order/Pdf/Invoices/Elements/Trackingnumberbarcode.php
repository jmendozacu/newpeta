...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Trackingnumberbarcode extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    const TRACKING_BARCODE_DEFAULT_NUDGE_X = -19;
    const TRACKING_BARCODE_DEFAULT_NUDGE_Y = 25;
    ...
    public function showBarcode() {
        ...
        $barcode_font_size = $this->_getConfig('font_size_barcode_order', 12, false, $this->getWonder(), $this->getStoreId());
        ...
        $tracking_number_barcode_nudge[0] += self::TRACKING_BARCODE_DEFAULT_NUDGE_X;
        $tracking_number_barcode_nudge[1] += self::TRACKING_BARCODE_DEFAULT_NUDGE_Y;
	    ...
	    if($tracking_number != ''){
		    ...
		    if($barcode_font_size > 18)
				$barcode_font_size = 15;
            ...
            $page->drawRectangle(($x - 5 + $tracking_number_barcode_nudge[0]), ($y + $tracking_number_barcode_nudge[1] ), ($x + $barcodeWidth + 2 + $tracking_number_barcode_nudge[0]), ($y + ($barcode_font_size * 1.4) + $tracking_number_barcode_nudge[1]));
			...
			elseif($barcode_font_size_action >24 && $barcode_font_size_action <= 36){
                $page->drawText($barcodeString, ($x + $tracking_number_barcode_nudge[0]), ($y + $tracking_number_barcode_nudge[1] + 19), 'CP1252');
