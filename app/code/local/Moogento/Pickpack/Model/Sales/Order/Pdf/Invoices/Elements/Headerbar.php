...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Headerbar extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    function showHeaderBar2() {
            ...
            switch ($this->generalConfig['fill_bars_subtitles']) {
                ...
                case 1:
                    if ($invoice_title_linebreak <= 1 && ($line_widths[0] > 0 || $line_widths[1] > 0)) {
                        $bottom_fillbar = ceil($this->subheader_start - ($this->generalConfig['font_size_subtitles'] / 2)) - $fillbar_padding[1] + $this->packingsheetConfig['pickpack_title_pattern_nudge'][1];
                        $this->titlebar_base_y = $bottom_fillbar;
                        $top_fillbar = ceil($this->subheader_start + $this->generalConfig['font_size_subtitles'] + 2) + $fillbar_padding[0] + $this->packingsheetConfig['pickpack_title_pattern_nudge'][1];
        ...
        if ($this->packingsheetConfig['pickpack_title_pattern'] != '') {
            ...
            $this->prepareTitleBarText($order);
            $page->drawText($this->order_number_display, $title_start_X  + $this->packingsheetConfig['pickpack_title_pattern_nudge'][0], $this->subheader_start + $this->packingsheetConfig['pickpack_title_pattern_nudge'][1], 'UTF-8');
    ...
    private function printTopLineTitleBar($page){
        ...
        /*adding value nudgeY for title bar*/
        $page_title_nuge_Y = $this->packingsheetConfig['pickpack_title_pattern_nudge'][1];
        $top_fillbar = ceil($this->headerBarXY[1] + $this->generalConfig['font_size_subtitles'] + 2) + $fillbar_padding[0] + $page_title_nuge_Y;
        if($line_widths[0] > 0){
            $background_color_subtitles_zend = new Zend_Pdf_Color_Html($this->generalConfig['background_color_subtitles']);
            $page->setFillColor($background_color_subtitles_zend);
            $page->setLineColor($background_color_subtitles_zend);
    ...
    private function printBottomLineTitleBar($page){
        ...
        /*adding value nudgeY for title bar*/
        $page_title_nuge_Y = $this->packingsheetConfig['pickpack_title_pattern_nudge'][1];
        $bottom_fillbar = ceil($this->headerBarXY[1] - ($this->generalConfig['font_size_subtitles'] / 2)) - $fillbar_padding[1] + $page_title_nuge_Y;
    ...
    private function printTitleBarBackGroundColor($page){
        ...
        $page->setLineColor($background_color_subtitles_zend);
        $page->setLineWidth(0.5);
        $page_title_nuge = $this->packingsheetConfig['pickpack_title_pattern_nudge'];
        $x1 = $pageConfig['padded_left'];
        $y1 = ceil($this->headerBarXY[1] - ($this->generalConfig['font_size_subtitles'] / 2)) + $page_title_nuge[1];
        $x2 = $pageConfig['padded_right'];
        $y2 = ceil($this->headerBarXY[1] + $this->generalConfig['font_size_subtitles'] + 2) + $page_title_nuge[1];
