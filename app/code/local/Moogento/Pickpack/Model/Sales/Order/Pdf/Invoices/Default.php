...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Default extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    public $current_order_first_page; //for store first page of current order
    public $current_order_page_count; //for store number of page for current order
    public $orders_page_count; //for store number of page for all order
    ...
    public function getPdfDefault($orders = array(), $from_shipment = 'order', $invoice_or_pack = 'pack', $order_invoice_id = '', $shipment_ids = '',$order_items_arr = array()) {
        ...
        //set current number of page for all order to 0;
        $this->orders_page_count = 0;
    ...
    public function getPdfOrder($orderSingle, $supplier = null, $from_shipment = 'order', $invoice_or_pack = 'pack', $order_invoice_id = '', $shipment_ids = '',$order_items_arr = array()) {
        ...
        //set current order page number to 0;
        $this->current_order_page_count = 0;
        ...
        do {
            ...
            if ((isset($this->sku_supplier_item_action_master[$order_id]) && $this->sku_supplier_item_action_master[$order_id] == 'keep' && $keep_supplier_order && $keep_supplier_login) || !$isSplitSupplier) {
                ...
                if ($packingsheetConfig['pickpack_packbarcode']){
                ...
                if (isset($packingsheetConfig['pickpack_logo_position']) && $packingsheetConfig['pickpack_logo_position'] != 'fullwidth') {
                ...
                if(($company_address_yn == 1) || ($company_address_yn == 'yesgroup')) {
                ...
                if(isset($tracking_number_barcode_yn)){
                ...
                if ($packingsheetConfig['pickpack_return_address_yn'] == 1
                || $packingsheetConfig['pickpack_return_address_yn'] == 'yesgroup') {
                ...
                //show aitoc order comment at bottom of shipping address
                if ($packingsheetConfig['notes_yn'] == 2){
                    if (Mage::helper('pickpack')->isInstalled('Aitoc_Aitcheckoutfields'))
                        $orderNoteElement->showAitocOrderComments();
                }else{
                ...
                if(($notes_yn == 1) && ($notes_position == 'yesshipping')) {
                    ...
                    $customerCommentsElement->subheader_start = $subheader_start;
                ...
                /**PRINTING ITwebexperts Payperrentals Date **/
                if (Mage::helper('pickpack')->isInstalled('ITwebexperts_Payperrentals') && $packingsheetConfig['show_sales_igniter_rental_date']) {
                    $ITwebexperts_Payperrentals_Element = Mage::getModel('pickpack/sales_order_pdf_invoices_elements_module_itwebexperts_payperrentals', array($this, $order));
                    $ITwebexperts_Payperrentals_Element->generalConfig = $generalConfig;
                    $ITwebexperts_Payperrentals_Element->packingsheetConfig = $packingsheetConfig;
                    $ITwebexperts_Payperrentals_Element->y = $this->y;
                    $ITwebexperts_Payperrentals_Element->subheader_start = $subheader_start;
                    $ITwebexperts_Payperrentals_Element->printSingleDate();
                    $this->is_print_ITwebexperts_Payperrentals_Date = $ITwebexperts_Payperrentals_Element->is_print_ITwebexperts_Payperrentals_Date;
                    $this->y = $ITwebexperts_Payperrentals_Element->y;
                    $subheader_start = $ITwebexperts_Payperrentals_Element->subheader_start;
                }
                ...
                if (!is_null($this->is_print_ITwebexperts_Payperrentals_Date))
                    $productsElement->is_print_ITwebexperts_Payperrentals_Date = $this->is_print_ITwebexperts_Payperrentals_Date;
                ...
                $totalsBlock->packingsheetConfig = $packingsheetConfig;
                $totalsBlock->y = $this->y;
                $totalsBlock->minYTotal = $productsElement->getMinProductY();
                ...
                if (Mage::helper('pickpack')->isInstalled('Moogento_Cn22')) {
                    if($this->_getConfig('show_customs_declaration',0, false, $wonder, $storeId) == 1) {
                        ...
                        $moogentoCn22Element->current_order_first_page = $this->current_order_first_page;
                ...
                /***********PRINTING ORDER DISCOUNT MESSAGE***********/
                $discount_yn = $this->_getConfig('total_discount_yn', '1', false, $wonder, $storeId);
                $discount_message = $this->_getConfig('discount_congrats_note', '', false, $wonder, $storeId);
                $discount_message = trim($discount_message);
                $prices_yn = Mage::helper('pickpack/config')->getConfig('prices_yn', 0, false, $wonder, $storeId);

                if ($discount_yn && $prices_yn && $discount_message) {
                    $discountNoteElement = Mage::getModel('pickpack/sales_order_pdf_invoices_elements_discountnote', array($this, $order));
                    $discountNoteElement->showDiscountNote($discount_message, $this->y);
                }
            }
    ...
    public function nooPage($page_size = '') {
        ...
        if ($this->current_order_page_count == 0){
            $this->current_order_first_page = $page;
        }
        $this->current_order_page_count += 1;
