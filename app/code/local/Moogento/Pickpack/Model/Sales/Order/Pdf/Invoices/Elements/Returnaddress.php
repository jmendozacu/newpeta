...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Returnaddress extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    public function showLogo1() {
        ...
        if ($image) {
            ...
            $return_image_dimension = explode(",", $this->_getConfig('pickpack_logo_dimension','180,120', false, $this->getWonder(), $storeId));
            ...
            if ((($image_ext == 'jpg') || ($image_ext == 'jpeg') || ($image_ext == 'png') || ($image_ext == 'PNG')) && (is_file($filename))) {
				...
				$minY[] = 0;
                if(isset($return_logo_XY[1]))
					$minY[] += $return_logo_XY[1];
                if(isset($return_image_dimension[1]))
					$minY[] += $return_image_dimension[1];
            }
            unset($image);
            unset($image_ext);
    ...
    public function showReturnAddress() {
        ...
        if ($return_address_yn == 'yesgroup') {
	        ...
	        if (Mage::helper('pickpack')->isInstalled('Moogento_CourierRules') && is_object($order->getShippingAddress())) {
                $zoneIds = array(
                    'group1' => $this->_getConfig('pickpack_return_address_group1_zone', '', false, $wonder, $storeId),
                    'group2' => $this->_getConfig('pickpack_return_address_group2_zone', '', false, $wonder, $storeId),
                    'group3' => $this->_getConfig('pickpack_return_address_group3_zone', '', false, $wonder, $storeId),
                );

                foreach ($zoneIds as $code => $zoneId) {
                    if ($zoneId) {
                        $zone = Mage::getModel('moogento_courierrules/zone')->load($zoneId);
                        if ($zone->validate($order)) {
                            $this->returnAddress = $this->_getConfig('pickpack_return_address_' . $code, '', false, $wonder, $storeId);
                        }
                    }
                }
	        }
        ...
        $rotate_return_address = isset($this->packingsheetConfig['rotate_return_address']) ? $this->packingsheetConfig['rotate_return_address'] : 0;
