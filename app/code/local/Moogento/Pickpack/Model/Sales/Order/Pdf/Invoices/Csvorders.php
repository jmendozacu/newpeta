...
{ship_country}';
        ...
        foreach ($order_collection as $order) {
            ...
            if (Mage::helper('pickpack')->isInstalled('Ess_M2ePro')) {
                ...
                //get marketplace_id and marketplace_order_id
                $m2epro_order_collection = Mage::getModel('M2ePro/Order')
                    ->getCollection()
                    ->addFieldToFilter('magento_order_id', array('eq' => $order->getId()));

                if ($m2epro_order_collection->getSize() != 0){

                    $m2epro_order = $m2epro_order_collection->getFirstItem();
                    $marketplace_id = $m2epro_order->getData("marketplace_id");
                    $m2epro_order_id = $m2epro_order->getId();
                    $row[$order_id]['marketplace_id'] = $marketplace_id;
                    $row[$order_id]['marketplace_order_id'] = $m2epro_order_id;

                    //get amazon_order_id
                    $m2epro_amazon_order_collection = Mage::getModel('M2ePro/Amazon_Order')
                        ->getCollection()
                        ->addFieldToFilter('order_id', array('eq' => $m2epro_order_id));
                    if ($m2epro_amazon_order_collection->getSize() != 0){
                        $amazon_order = $m2epro_amazon_order_collection->getFirstItem();
                        $row[$order_id]['amazon_order_id'] = $amazon_order->getData('amazon_order_id');
                    }

                    //get ebay_order_id
                    $m2epro_ebay_order_collection = Mage::getModel('M2ePro/Ebay_Order')
                        ->getCollection()
                        ->addFieldToFilter('order_id', array('eq' => $m2epro_order_id));
                    if ($m2epro_ebay_order_collection->getSize() != 0){
                        $ebay_order = $m2epro_ebay_order_collection->getFirstItem();
                        $row[$order_id]['ebay_order_id'] = $ebay_order->getData('ebay_order_id');

                        //get ebay_final_fee
                        $row[$order_id]['ebay_final_fee'] = $ebay_order->getFinalFee();

                        //get paypal_transaction_id and paypal_fee
                        $m2epro_ebay_order_externaltransaction_collection = Mage::getModel('M2ePro/Ebay_Order_ExternalTransaction')
                            ->getCollection()
                            ->addFieldToFilter('order_id', array('eq' => $m2epro_order_id));
                        if ($m2epro_ebay_order_externaltransaction_collection->getSize() != 0){
                            $m2epro_ebay_order_externaltransaction = $m2epro_ebay_order_externaltransaction_collection->getFirstItem();
                            $row[$order_id]['paypal_fee'] = $m2epro_ebay_order_externaltransaction->getData('fee');
                            $row[$order_id]['paypal_transaction_id'] = $m2epro_ebay_order_externaltransaction->getData('transaction_id');
                        }
                    }
            ...
            if (Mage::helper('pickpack')->isInstalled('Camiloo_Channelunity')) {
                ...
                $row[$order_id]['channel_unity_origin'] = implode(', ', $sources);
            }
