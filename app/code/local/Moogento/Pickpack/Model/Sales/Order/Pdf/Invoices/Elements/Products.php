...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Products extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    protected $x_pos_tax_per_item = 0;
	...
	*/
	public function getMinProductY() {
        ...
        if ($this->countPdfPages() == 0) {
            ...
            // Caculate space for Cn22
            // This will prevent product and total print over Cn22
            // but only applied when Cn22 don't nugde over half first page
            if (Mage::helper('pickpack')->isInstalled('Moogento_Cn22')) {
                if ($this->packingsheetConfig['show_customs_declaration']) {
                    $show_customs_declaration_nudge = $this->packingsheetConfig['show_customs_declaration_nudge'];
                    if ($show_customs_declaration_nudge[1] < $pageConfig['full_page_height'] / 2
                        &&
                        $show_customs_declaration_nudge[1] > $this->min_product_y
                    )
                        $this->min_product_y = $show_customs_declaration_nudge[1];
                }
            }
	...
	*/
	private function checkNewPageNeeded($specific_section = '', $specific_option = '', $caculate_y = null){
		$helper = Mage::helper('pickpack');
		$pageConfig = $this->getPageConfig();
		$generalConfig = $this->getGeneralConfig();
		
		if (is_null($caculate_y))
			$caculate_y = $this->y;
		
		if ( 
			($caculate_y < $pageConfig['page_bottom']) || 
			($caculate_y < $this->getMinProductY())
			) {
			$page = $this->newPage();
	...
	*/
    ...
    protected function preparePositionArrayX() {
        ...
        if(Mage::helper('pickpack')->isInstalled('ITwebexperts_Payperrentals'))
            if ($this->packingsheetConfig['show_sales_igniter_rental_date'] == 1)
                $columns_xpos_array['sales_igniter_rental_date_xpos'] = $this->packingsheetConfig['show_sales_igniter_rental_date_xpos'];
    ...
    public function showGridHeader() {
        ...
        if(Mage::helper('pickpack')->isInstalled('ITwebexperts_Payperrentals'))
            if($this->packingsheetConfig['show_sales_igniter_rental_date'] == 1)
                $this->_drawText(Mage::helper('sales')->__($this->packingsheetConfig['show_sales_igniter_rental_date_title']), ($this->packingsheetConfig['show_sales_igniter_rental_date_xpos'] + $this->first_item_title_shift_items), $this->y);
        ...
        if ($prices_yn != '0') {
            ...
            if ($this->packingsheetConfig['product_line_tax_yn']) {
                $this->_drawText(Mage::helper('pickpack')->__($this->packingsheetConfig['product_line_tax_title']), $this->packingsheetConfig['product_line_tax_title_xpos'], $this->y);
                $this->x_pos_tax_per_item = $this->packingsheetConfig['product_line_tax_title_xpos'];
            }
            if ($this->packingsheetConfig['product_line_total_yn'])
                $this->_drawText(Mage::helper('pickpack')->__($this->packingsheetConfig['product_line_total_title']), $this->packingsheetConfig['product_line_total_title_xpos'], $this->y);
        }
    ...
    public function showItem($product_build_value, $from_shipment = 'order', $invoice_or_pack = 'pack', $order_invoice_id = '', $shipment_ids = '', $order_items_arr = array()) {
        ...
        if( ($this->packingsheetConfig['new_pdf_per_name_yn'] == 0) 
            ...
            if ((!$order_invoice_id || $this->checkItemBelongInvoiceDetail($sku_real, $order_invoice_id)) && (!$shipment_ids || $this->checkItemBelongShipment($sku_real, $shipment_ids)) && $is_show_zero_qty) {
                ...
                if ($this->packingsheetConfig['show_zero_qty_options'] == 1 && $this->packingsheetConfig['show_qty_options'] == 2 && $product_build_value['qty_string'][2] == 0)
                    $this->fontColorBodyItem = Mage::helper('pickpack/config_color')->getPdfColor('grayout_color');
                else if ($this->packingsheetConfig['show_zero_qty_options'] == 1 && $this->packingsheetConfig['show_qty_options'] != 2 && (int)$product_build_value['qty_string'] == 0)
                    $this->fontColorBodyItem = Mage::helper('pickpack/config_color')->getPdfColor('grayout_color');
                else
		...
		$this->printingImage($product_build_value);
                ...
                $this->_setFont($page, $generalConfig['font_style_body'], $generalConfig['font_size_body'], $generalConfig['font_family_body'], $generalConfig['non_standard_characters'], $this->fontColorBodyItem);
                if($this->packingsheetConfig['show_qty_options'] == 2){
                ...
                if ($prices_yn != 0){
                    ...
                    $priceBlock->y = $this->y;
                    $priceBlock->prepareConfigObject();
                    ...
                    if ($priceBlock->showBaseValue)
                        if (!isset($next_product_line_ypos) || $next_product_line_ypos > $priceBlock->y - 1.5 * $this->generalConfig['font_size_body'])
                            $next_product_line_ypos = $priceBlock->y - 3 * $this->generalConfig['font_size_body'];
                }
                /***************************PRINTING ALLOWANCE**********************/
                if($this->packingsheetConfig['show_allowance_yn'] == 1)
                ...
                $max_name_length = $next_col_to_product_x - $this->packingsheetConfig['product_name_x_pos'] - $this->productXInc;
                ...
                if($name != "" && !$this->hide_bundle_parent_f){
                    ...
                    if ($this->packingsheetConfig['product_name_trim_yn']) {
                        if ($this->packingsheetConfig['show_product_name'] == 1){
                            ...
                            $temp_y = $this->y;
                            $name_trim = Mage::helper('pickpack/font')->trimTextOnRealFont($name, $generalConfig['font_family_body'], $generalConfig['font_style_body'], $generalConfig['font_size_body'], $max_name_length - 5);
                        ...
                        if ($this->packingsheetConfig['show_product_name'] == 1) {
                            $temp_y = $this->y;
                            $multiline_name_array = Mage::helper('pickpack/font')->wordWrapOnRealFont($name, $generalConfig['font_family_body'], $generalConfig['font_style_body'], $generalConfig['font_size_body'], $max_name_length - 5);
                            foreach ($multiline_name_array as $name_in_line) {
                                $line_count++;
                ...
                /***************************PRINTING SALES IGNITER RENTAL CUSTOM COLUMN**********************/
                if (Mage::helper('pickpack')->isInstalled('ITwebexperts_Payperrentals') && $this->packingsheetConfig['show_sales_igniter_rental_date']) {
                    if (!is_null($this->is_print_ITwebexperts_Payperrentals_Date) && $this->is_print_ITwebexperts_Payperrentals_Date == false){
                        $customColumnElement = Mage::getModel('pickpack/sales_order_pdf_invoices_elements_products_customcolumn', array($this, $order));
                        $customColumnElement->generalConfig = $this->generalConfig;
                        $customColumnElement->packingsheetConfig = $this->packingsheetConfig;
                        $customColumnElement->product_build_value = $product_build_value;
                        $customColumnElement->y = $this->y;
                        if (isset($next_product_line_ypos))
                            $customColumnElement->next_product_line_ypos = $next_product_line_ypos;
                        $customColumnElement->printSalesIgniterRentalColumn();
                        $next_product_line_ypos = $customColumnElement->next_product_line_ypos;
                    }
                }
...
//                                }
                                ...
                                /***************************PRINTING BUNDLE TAX PER ITEM**********************/
                                if( $child->getData('tax_amount') > 0 ){
                                    $taxPerItem = Mage::helper('core')->currency($child->getData('tax_amount'), true, false);
                                    $this->_drawText($taxPerItem, $this->x_pos_tax_per_item, $this->y );
                                }
                ...
                if (isset($next_product_line_ypos) && ($next_product_line_ypos > 0) && $this->y > $next_product_line_ypos)
    ...
    private function skuWordwrap($minDistanceSku, $font_size_sku, $sku_print) {
        ...
        if (strlen($sku_print) > 0) {
            ...
            if (strlen($sku_print) > $max_chars)
                $chunks = mb_wordwrap_array($sku_print, $max_chars);
            else
                $chunks[] = $sku_print;
    ...
    protected function printingImage($product_build_value) {
        ...
        if ($this->packingsheetConfig['product_images_yn'] == 1 && $this->sku_ProductId[$product_sku] != '' && !$this->hide_bundle_parent_f) {
            ...
            $img_dimension = $helper->getWidthHeightImage($product, $product_images_source_res, $product_images_maxdimensions);
            if (is_array($img_dimension) && count($img_dimension)) {
                $this->img_width = $img_dimension[0];
                $this->img_height = $img_dimension[1];
            }
            ...
            foreach ($imagePaths as $imagePath) {
                ...
                if ($imagePath != '') {
                    ...
                    try {
						...
						if($this->checkNewPageNeeded('','',$y1) === true) {
							$page = $this->getPage();
                    ...
                    if ($this->flag_image_newpage)
                        $this->after_print_image_y_newpage = $this->after_print_image_y;
                }
            }
            if ($this->has_shown_product_image == 0) {
