...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Giftmessage extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    public function showBottomOrderGiftMessage() {
        ...
        if($gift_message != ''){
            if ($gift_message_yn == 'yesbox') {
                ...
                $x2_right = $positional_message_box_fixed_position[0] + $positional_message_box_fixed_position_dimension_x;
                $gift_msg_array = $this->splitWordsToArrayBasedOnMaxWidth($gift_message, $positional_message_box_fixed_position_dimension_x, $font_size_choice, $font_family_choice);
            ...
            if ($gift_message_yn == 'yesnewpage') {
                ...
                if ($bkg_color_choice != '#FFFFFF') {
                    $colorHtml = new Zend_Pdf_Color_Html($bkg_color_choice);
                    $page->setFillColor($colorHtml);
                    $page->setLineColor($colorHtml);
    ...
    public function showBottomProductGiftMessage() {
        ...
        if(isset($this->gift_message_array['items']) && ($gift_message_combined)) {
            ...
            if ($product_gift_message_yn == 'yesbox') {
                ...
                $font_color_choice = $generalConfig['font_color_comments'];
                $x2_right = $positional_message_box_fixed_position[0] + $positional_message_box_fixed_position_dimension_x;
                $gift_msg_array = $this->splitWordsToArrayBasedOnMaxWidth($gift_message_combined, $positional_message_box_fixed_position_dimension_x, $font_size_choice, $font_family_choice);
                $message_line_spacing = $font_size_choice * 1.5;
	...
	*/
    ...
    protected function drawGiftMessageText($gift_msg_array, $msgX, $font_size, $y, $page, $padding_yn) {
		$msgX += $this->getBoxPadding($padding_yn);
		$y -= $this->getBoxPadding($padding_yn);
        Mage::log($gift_msg_array,null,'gift.log');
		foreach ($gift_msg_array as $gift_msg_line) {
    ...
    public function showRepeatGiftMessage() {
        ...
        if(isset($this->gift_message_array['items']) && ($gift_message_combined = $this->getProductGiftMessage($this->gift_message_array))){
            ...
            $gift_msg_pro_array = $this->splitWordsToArrayBasedOnMaxWidth($gift_message_combined, $positional_message_box_fixed_position_dimension_x, $generalConfig['font_size_comments'], $generalConfig['font_family_comments']);
        }
        $gift_message = '';
        if((!is_null($gift_message_id) || $this->giftWrap_info['message'] != NULL || $this->giftWrap_info['wrapping_paper'] != NULL)){
        ...
        if($gift_message != ''){
            $message_character_breakpoint = $positional_message_box_fixed_position_dimension_x;
            $gift_message = wordwrap($gift_message, $message_character_breakpoint, "\n", false);
            $gift_msg_array = $this->splitWordsToArrayBasedOnMaxWidth($gift_message, $positional_message_box_fixed_position_dimension_x, $generalConfig['font_size_comments'], $generalConfig['font_family_comments']);
        }
        ...
        if($gift_msg_combined != null){
            ...
            $x2_right = $positional_remessage_box_fixed_position[0] + $positional_message_box_fixed_position_dimension_x;
