...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Labelzebra_OrderNotes extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Elements_Abstract
{
    ...
    private function getOrderNotesData(){
        ...
        if ($order->getStatusHistoryCollection(true)) {
            ...
            $font_temp = Mage::helper('pickpack/font')->getFontName2($this->generalConfig['font_family_comments'], $this->generalConfig['font_style_message'], $this->_general['non_standard_characters']);
            $character_breakpoint = Mage::helper('pickpack/font')->getMaxCharMessage($this->page_padding['right'], $this->generalConfig['font_size_message'], $font_temp, $this->page_padding['left']);
            ...
            foreach ($notes as $_item) {
                ...
                if (Mage::helper('pickpack')->isInstalled('Ess_M2ePro')) {
                    ...
                    $_item['comment'] = strip_tags($_item['comment']);
                }
    ...
    private function checkForNewZebraPage(){
        ...
        if ($this->y - $height_of_block < $this->page_padding['bottom']){
            $page_dimension        = $page_width . ':' . $page_height;
    ...
    private function wordWrapNoteComments($note_line,$character_breakpoint){
        ...
        foreach ($note_line_break as $note_line_each){
            if ($note_line_each != "") {
                ...
                $comment_array = mb_wordwrap_array($note_line_each, $character_breakpoint);
    ...
    private function printOrderNotes($note_line = array()){
        ...
        if ($this->line_count_note) {
            ...
            if (($bkg_color_message != '') && ($bkg_color_message != '#FFFFFF')) {
                ...
                $pdf->drawRectangle($page,$x1, $y1, $x2, $y2);
                $this->y -= $this->generalConfig['font_size_message'] * 1.2;
            }else $this->y -= $this->generalConfig['font_size_message'] * 1.2;
            ...
            if ($this->zebralabelConfig['order_notes_title'] != ''){
                ...
                $pdf->drawText($page,$helper->__($this->zebralabelConfig['order_notes_title']), ($this->page_padding['left'] + 10), $this->y, 'UTF-8');
            ...
            foreach ($note_line as $note){
                foreach ($note['comment'] as $comment_line){
                    $pdf->drawText($page,$comment_line, ($this->page_padding['left'] + 10), $this->y, 'UTF-8');
                    $this->y -= $this->generalConfig['font_size_message'];
                }
            }
