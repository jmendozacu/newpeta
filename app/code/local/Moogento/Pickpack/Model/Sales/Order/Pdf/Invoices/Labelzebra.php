...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Labelzebra extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
	...
	const ORDER_ID_BARCODE_NUDGE_X = 0;
	const ORDER_ID_BARCODE_NUDGE_Y = -90;
    ...
    public function getConfigValue($storeId){
        ...
        $page_size = $this->zebralabelConfig['nudge_dimension_zebra'];
	...
	*/
    ...
    public function getLabelzebra($orders = array()) {
        ...
        $dimension_zebra = explode(",", $this->_getConfig('nudge_dimension_zebra', '432,288', false, 'label_zebra'));
        $label_width     = $dimension_zebra[0];
        $label_height    = $dimension_zebra[1];
        $page_top        = $label_height - 2;
        $padded_right    = $label_width - 10;
        $paper_width     = $label_width;
        $paper_height    = $label_height;
        if ($rotate_label == 0) {
            $page_dimension        = $label_width . ':' . $label_height;
            $rotate_point[0]       = 0;
            $rotate_point[1]       = 0;
            $settings['page_size'] = $page_dimension;
            $this->pageSettings = $settings;
            $page                  = $this->newPageZebra($settings);
        } elseif ($rotate_label == 1) {
            $page_dimension        = $label_height . ':' . $label_width;
            $rotate_point[0]       = 0;
            $rotate_point[1]       = 0;
            $settings['page_size'] = $page_dimension;
            $this->pageSettings = $settings;
        ...
        foreach ($orders as $orderSingle) {
            ...
            if (Mage::helper('pickpack')->isInstalled('Moogento_CourierRules') && ($this->_getConfig('use_courierrules_shipping_label',0, false, 'label_zebra', $storeId) == 1)) {
                try{
                    ...
                    $show_courierrules_label_dimension[0] = $dimension_zebra[0];
                    $show_courierrules_label_dimension[1] = $dimension_zebra[1];
...
//                    $this->_config['nudge_dimension_zebra'][1]  : the height of page

                    $image_line_x = $this->_base_position['X'] + $nudge_shipping_bg[0];
                    $image_line_y = $this->_base_position['Y'] + $this->_config['nudge_dimension_zebra'][1] + $nudge_shipping_bg[1];
                ...
                $this->printOrderIdAndBarcode($temp_x,$temp_y,$label_width);
                /******************* END ORDER ID AND BARCODE *******************/
                ...
                /*************************** PRINT ZIPCODE BARCODE *******************************/
                $this->printZipcodeBarcode($temp_x, $subheader_start, $label_width);
                /*************************** END PRINT ZIPCODE BARCODE *******************************/
                ...
                if ($this->zebralabelConfig['tracking_number_barcode_yn']){
    ...
    public function drawText($page,$text = '', $x = 0, $y = 0, $charEncoding = 'UTF-8'){
        if (isset($page)){
    ...
    public function drawImage($page, $image, $x1, $y1, $x2, $y2) {
        if (isset($page)){
    ...
    public function drawRectangle($page,$x1, $y1, $x2, $y2, $fillType = Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE){
        if (isset($page)){
    ...
    //this function will draw Rectangle with right position when we rotate pdf
    public function drawLine($page, $x1, $y1, $x2, $y2){
        if (isset($page)){
    ...
    private function setLabelZebraConfig($storeId) {
        ...
        $this->_config['nudge_dimension_zebra'] = explode(",", $this->_getConfig('nudge_dimension_zebra', '432,288', false, 'label_zebra',$storeId));
        $this->_label_width = $this->_config['nudge_dimension_zebra'][0];
        $this->_label_height = $this->_config['nudge_dimension_zebra'][1];
        ...
        if (!isset($this->_base_position['X'])||!isset($this->_base_position['Y'])){
            if ($this->_config['rotate_label'] == 1){
                ...
                $this->_base_position['Y'] = -intval($this->_config['nudge_dimension_zebra'][1]);
            }elseif ($this->_config['rotate_label'] == 2){
                $this->_base_position['X'] = -intval($this->_config['nudge_dimension_zebra'][0]);
    ...
    public function newPageZebra(array $settings = array()) {
        $pageSize     = !empty($settings['page_size']) ? $settings['page_size'] : $this->pageSettings['page_size'];
...
//            $tracking_number_barcode_nudge = array(0,0);
    ...
    public function showTrackingNumberBarcode($page, $order, $x, $y) {
        ...
        $barcode_font_size = $this->zebralabelConfig['tracking_number_barcode_font_size'];
        $tracking_number_barcode_nudge = $this->zebralabelConfig['tracking_number_barcode_nudge'];
    ...
    private function printOrderIdAndBarcode($temp_x,$temp_y,$label_width){
        $page = $this->getPage();
        $order = $this->getOrder();
        ...
        $this->zebralabelConfig['nudge_order_id_barcode'][0] += self::ORDER_ID_BARCODE_NUDGE_X;
        $this->zebralabelConfig['nudge_order_id_barcode'][1] += self::ORDER_ID_BARCODE_NUDGE_Y;

        //print barcode of order id
        if ($this->zebralabelConfig['show_order_id_barcode_yn'] == 1) {
            $barcodeString = Mage::helper('pickpack/barcode')->convertToBarcodeString($order_id, $this->generalConfig['barcode_type']);
            //$barcodeWidth = 1.5 * $this->parseString($barcodeString, Zend_Pdf_Font::fontWithPath($this->action_path . $this->generalConfig['font_family_barcode']), $barcode_font_size);
            $page->setLineColor($black_color);
            $page->setFillColor(new Zend_Pdf_Color_Rgb(0, 0, 0));
            $page->setFont(Zend_Pdf_Font::fontWithPath($this->action_path . $this->generalConfig['font_family_barcode']), $this->zebralabelConfig['order_id_barcode_font_size']);

            $order_barcode_x = $label_width/3*2 + $this->zebralabelConfig['nudge_order_id_barcode'][0];
            $order_barcode_y = ($temp_y + $this->zebralabelConfig['nudge_order_id_barcode'][1]);

            $this->drawText($page,$barcodeString, $order_barcode_x, $order_barcode_y, 'CP1252');
        }

        //print order id
        if ($this->zebralabelConfig['label_show_order_id_yn'] == 1) {
            if ($this->zebralabelConfig['show_order_id_barcode_yn'] == 1){
                $font_temp = Mage::helper('pickpack/font')->getFont($this->zebralabelConfig['font_style_label'], $this->zebralabelConfig['font_size_order_id'], $this->zebralabelConfig['font_family_label']);
                $order_id_width = $this->parseString($barcodeString, $font_temp, $this->zebralabelConfig['font_size_order_id']);

                $page->setFillColor($white_color);
                $page->setLineColor($white_color);
                $this->drawRectangle($page,($order_barcode_x + 10), ($order_barcode_y + 10), ($order_barcode_x + 30 + $order_id_width), ($order_barcode_y - 10));

                $order_id_xpos = $order_barcode_x + 20;
                $order_id_ypos = $order_barcode_y;
            }else{
                $order_id_xpos = $temp_x + $this->zebralabelConfig['nudge_order_id'][0];
                $order_id_ypos = $temp_y + $this->zebralabelConfig['nudge_order_id'][1];
            }

            $this->_setFont($page, $this->zebralabelConfig['font_style_label'], $this->zebralabelConfig['font_size_order_id'], $this->zebralabelConfig['font_family_label'], 0, $this->zebralabelConfig['font_color_label']);
            $page->setFillColor($black_color);

            $this->drawText($page,$order_id, $order_id_xpos, $order_id_ypos, 'UTF-8');
        }
    }

    public function printZipcodeBarcode($temp_x, $subheader_start, $label_width){
        $page = $this->getPage();
        $order = $this->getOrder();
        ...
        if ($this->zebralabelConfig['show_zipcode_barcode'] == 1 && $zipcode != '') {
            $barcodeString     = Mage::helper('pickpack/barcode')->convertToBarcodeString($zipcode, $this->generalConfig['font_family_barcode']);
            //$barcodeWidth = 1.5 * $this->parseString($barcodeString, Zend_Pdf_Font::fontWithPath($this->action_path . $this->generalConfig['font_family_barcode']), $barcode_font_size);

            $barcode_x = $label_width/3*2 + $this->zebralabelConfig['zipcode_barcode_nudge'][0];
            $barcode_y = $subheader_start + $this->zebralabelConfig['zipcode_barcode_fontsize'] + $this->zebralabelConfig['zipcode_barcode_nudge'][1];

            $page->setLineColor($black_color);
            $page->setFillColor($black_color);
            $page->setFont(Zend_Pdf_Font::fontWithPath($this->action_path . $this->generalConfig['font_family_barcode']), $this->zebralabelConfig['zipcode_barcode_fontsize']);
            $this->drawText($page,$barcodeString, $barcode_x, $barcode_y, 'CP1252');
        }
    }
}
