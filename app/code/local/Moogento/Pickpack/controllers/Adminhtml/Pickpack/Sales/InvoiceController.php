...
class Moogento_Pickpack_Adminhtml_Pickpack_Sales_InvoiceController extends Mage_Adminhtml_Sales_InvoiceController
{
    ...
    {
        if (isset($this->_invoiceCache[$id])) {
            return $this->_invoiceCache[$id];
        }
        $this->_invoiceCache[$id] = Mage::getModel('sales/order_invoice')->load($id);
        if (!$this->_invoiceCache[$id]->getId()) {
            $this->_invoiceCache[$id] = false;
        }
    ...
    protected function _getOrderIds($invoice_ids) {
        ...
        foreach ($invoice_ids as $invoice_id) {
            $invoice = $this->_getInvoice($invoice_id);
    ...
    public function mooinvoiceAction() {
        ...
        if (!empty($invoiceIds)) {
            ...
            foreach ($invoiceIds as $invoiceId) {
                $invoice = $this->_getInvoice($invoiceId);
                if ($invoice) {
                    $orderIds = array($invoice->getOrderId());
                    ...
                    if (count($orderIds)) {
                        $subPdf = Mage::getModel('pickpack/sales_order_pdf_invoices_default')->getPdfDefault($orderIds, $from_shipment, 'invoice', $invoiceId);
            ...
            Mage::dispatchEvent(
                ...
                array('order_ids' => $allOrderIds)
            );
            Mage::helper('pickpack/print')->processPrint($allOrderIds, 'invoice');
            if ($hasContent) {
    ...
    public function packAction() {
        ...
        if (!empty($invoiceIds)) {
            ...
            Mage::helper('pickpack/print')->processPrint($allOrderIds, 'pack');
            if ($hasContent) {
