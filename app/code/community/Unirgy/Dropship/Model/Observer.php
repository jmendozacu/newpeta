...
class Unirgy_Dropship_Model_Observer extends Varien_Object
{
    ...
    */
    public function sales_order_save_after($observer)
    {
        if (!$observer->getEvent()->getOrder()->getNoDropshipFlag()) {
            if (!Mage::helper('udropship')->isSalesFlat()) {
                $this->__sales_order_save_after($observer);
            }
        }
    }
    ...
    protected function __sales_order_save_after($observer)
    {
        /** @var Unirgy_Dropship_Model_Mysql4_Helper $rHlp */
        $rHlp = Mage::getResourceSingleton('udropship/helper');
        $useLock = Mage::getStoreConfigFlag('udropship/admin/for_update_split_check');
        ...
        try {
            ...
            $this->protected_sales_order_save_after($observer);
            if ($useLock) $rHlp->commit();
        } catch (Exception $e) {
            if ($useLock) $rHlp->rollBack();
            throw $e;
        }
    }
    public function protected_sales_order_save_after($observer)
    {
        $order = $observer->getEvent()->getOrder();
        $store = $order->getStore();
        $storeId = $store->getId();
        $enableVirtual = Mage::getStoreConfig('udropship/misc/enable_virtual', $order->getStoreId());
        $forcedActive = Mage::getStoreConfigFlag('carriers/udropship/force_active', $store);
...
#Mage::log($order->debug());
        ...
        */
        $rHlp = Mage::getResourceSingleton('udropship/helper');
        $oUdStatus = Mage::getStoreConfigFlag('udropship/admin/for_update_split_check')
            ? $rHlp->loadModelFieldForUpdate($order, 'udropship_status')
            : $rHlp->loadModelField($order, 'udropship_status');
        if ($order->getUdropshipStatus()==Unirgy_Dropship_Model_Source::ORDER_STATUS_PENDING
            && $oUdStatus!=Unirgy_Dropship_Model_Source::ORDER_STATUS_PENDING
        ) {
            $order->setUdropshipStatus($oUdStatus);
        }
        if ($order->getUdropshipStatus()!=Unirgy_Dropship_Model_Source::ORDER_STATUS_PENDING) {
            if ($order->getUdropshipStatus()==Unirgy_Dropship_Model_Source::ORDER_STATUS_NOTIFIED
...
#Mage::log($shippingDetails);
        ...
        try {
            ...
            Mage::helper('udropship/protected')->splitOrder($order);
        } catch (Exception $e) {
    ...
    protected function _cancelOrder($order)
    {
        foreach ($order->getShipmentsCollection() as $shipment) {
            $shipment->setUdropshipStatus(Unirgy_Dropship_Model_Source::SHIPMENT_STATUS_CANCELED)->save();
        }
        $order->setUdropshipStatus(Unirgy_Dropship_Model_Source::ORDER_STATUS_CANCELED)->save();
        return $this;
    }
    ...
    {
        $shippingMethod = Mage::helper('udropship')->explodeOrderShippingMethod($order);
        // fix googlecheckout shipping method
        if ($shippingMethod[0]=='googlecheckout') {
            ...
            if (!$updated) {
                ...
                return false; // couldn't find the correct shipping method
            }
        }
        return true;
    }
    ...
    public function getOrderVendorRates($order)
    {
        ...
        $details = false;
        try {
            $details = Mage::helper('core')->jsonDecode($shippingDetails);
        } catch (Exception $e) {}
