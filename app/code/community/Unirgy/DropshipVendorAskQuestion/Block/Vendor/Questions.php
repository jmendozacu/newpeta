    ...
    protected function _applyRequestFilters($collection)
    {
        ...
        if (!is_null($param) && $param!=='') {
            if (Mage::helper('udropship')->compareMageVer('1.9.1.1', '1.14.1.0')) {
                if ($param) {
                    $collection->addFieldToFilter('answer_text_length', array('gt' => 0));
                } else {
                    $collection->addFieldToFilter(array('answer_text','answer_text_length'), array(array('null' => 1), array('eq' => 0)));
                }
            } else {
                if ($param) {
