        ...
        foreach ($items as $item) {
            ...
            if ($_children) {
                ...
                foreach ($childrenByPid as $pId => $_children) {
                    ...
                    } elseif ($stockItem && $stockItem->getQtyIncrements() && (Mage::helper('core')->getExactDivision($childQtyUsed, $stockItem->getQtyIncrements()) != 0)) {
                        $allowedQtyError = true;
                        $error = true;
                        if ($item->getProductType()=='configurable') {
                            $message = Mage::helper('cataloginventory')->__('This product is available for purchase in increments of %s only.', $stockItem->getQtyIncrements() * 1);
                        } else {
                            $message = Mage::helper('cataloginventory')->__('%s is available for purchase in increments of %s only.', $childName, $stockItem->getQtyIncrements() * 1);
                        }
                ...
                } elseif ($stockItem && $stockItem->getQtyIncrements() && (Mage::helper('core')->getExactDivision($qtyToCheck, $stockItem->getQtyIncrements()) != 0)) {
                    $allowedQtyError = true;
                    $item->setHasError(true);
                    $item->setMessage(Mage::helper('cataloginventory')->__('This product is available for purchase in increments of %s only.', $stockItem->getQtyIncrements() * 1));
                } else {
                    if (empty($qtyUsed[$pId][$vId])) {
                        $qtyUsed[$pId][$vId] = 0;
                    }
