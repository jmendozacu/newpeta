...
#foreach ($requests as $r) var_dump($r); exit;
        ...
        $shipping = $hl->getShippingMethods($request->getWebsiteId());
        ...
        foreach ($requests as $vId=>$vRequests) {
            ...
            $systemMethods = $hl->getMultiSystemShippingMethodsByProfile($vendor, $request->getWebsiteId());
        ...
        if ($details && ($details = Mage::helper('core')->jsonDecode($details)) && !empty($details['methods'])) {
